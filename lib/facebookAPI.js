...
module.exports = {
  ...
  getGroups: async (id, token) => {
    const options = {
      method: "GET",
      uri: `https://graph.facebook.com/v4.0/${id}/groups`,
      qs: {
        access_token: token,
        fields: `
        icon,description,cover,id,name,email,created_time`
      }
    };
    return new Promise((resolve, reject) => {
      request(options, (err, res, body) => {
  ...
  getGroup: async (groupId, token) => {
    const options = {
      method: "GET",
      uri: `https://graph.facebook.com/v4.0/${groupId}`,
      qs: {
        access_token: token,
        fields: `
        cover, description, name, permissions`
      }
    };
    return new Promise((resolve, reject) => {
      request(options, (err, res, body) => {
